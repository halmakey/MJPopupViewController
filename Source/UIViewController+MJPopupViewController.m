...
#define kMJPopupAnimationType  @"kMJPopupAnimationType"
...
- (MJPopupViewAnimation)mj_animationType
{
    return [objc_getAssociatedObject(self, kMJPopupAnimationType) unsignedIntegerValue];
}

- (void)setMj_animationType:(MJPopupViewAnimation)mj_animationType
{
    objc_setAssociatedObject(self, kMJPopupAnimationType, [NSNumber numberWithUnsignedInteger:mj_animationType], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)presentPopupViewController:(UIViewController*)popupViewController animationType:(MJPopupViewAnimation)animationType option:(MJPopupOption)option dismissed:(void(^)(void))dismissed;
{
    ...
    self.mj_popupViewController.mj_animationType = animationType;

    [self addChildViewController:self.mj_popupViewController];
    [self presentPopupView:popupViewController.view animationType:animationType option:option dismissed:dismissed];
}
...
{
    [self presentPopupViewController:popupViewController animationType:animationType option:option dismissed:nil];
}

- (void)dismissPopupViewController
{
    UIView *sourceView = self.parentViewController.view;
    UIView *popupView = [sourceView viewWithTag:kMJPopupViewTag];
    UIView *overlayView = [sourceView viewWithTag:kMJOverlayViewTag];

    MJPopupViewAnimation animationType = self.mj_animationType;
...
{
    [self presentPopupView:popupView animationType:animationType option:MJPopupOptionTapBackgroundToClose dismissed:nil];
}

- (void)presentPopupView:(UIView*)popupView animationType:(MJPopupViewAnimation)animationType option:(MJPopupOption)option dismissed:(void(^)(void))dismissed
{
    ...
    if (option & MJPopupOptionTapBackgroundToClose) {
...
{
    [self.mj_popupViewController dismissPopupViewController];
}
...
- (void)slideViewIn:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView withAnimationType:(MJPopupViewAnimation)animationType
{
    ...
    [UIView animateWithDuration:kPopupModalAnimationDuration delay:0.0f options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        [self.mj_popupViewController didMoveToParentViewController:self];
    }];
}
...
- (void)slideViewOut:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView withAnimationType:(MJPopupViewAnimation)animationType
{
    ...
    [UIView animateWithDuration:kPopupModalAnimationDuration delay:0.0f options:UIViewAnimationOptionCurveEaseIn animations:^{
        popupView.frame = popupEndRect;
        parent.mj_popupBackgroundView.alpha = 0.0f;
    } completion:^(BOOL finished) {
        [popupView removeFromSuperview];
        [overlayView removeFromSuperview];
        [parent.mj_popupViewController removeFromParentViewController];
        parent.mj_popupViewController = nil;
